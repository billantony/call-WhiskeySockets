...
import { AudioFeederNoFFmpeg } from "./audio-feeder-noffmpeg.mjs";
...
export class ActiveCall extends EventEmitter {
    ...
    _emitAudioOut = (pcm) => { this.emit("audioOut", pcm); };
    /** @internal */
    _forceEnd = (reason) => {
        if (this.#ended)
            return;
...
export class VoipClient {
        ...
        await new Promise((resolveOpen, rejectOpen) => {
            ...
            const connectSocket = () => {
                ...
                this.#sock.ev.on("connection.update", (update) => {
                    console.log(`[BAILEYS-CALLER] Socket connection update:`, update);
                    if (update.qr) {
                    ...
                    if (update.connection === "open") {
                        console.log(`[BAILEYS-CALLER] Socket connection opened`);
                        opened = true;
                        process.removeAllListeners("uncaughtException");
                        resolveOpen();
                        return;
                    ...
                    if (update.connection === "close" && !opened) {
                        console.log(`[BAILEYS-CALLER] Socket connection closed before open:`, update.lastDisconnect);
                        const statusCode = update.lastDisconnect?.error?.output?.statusCode;
                        // Desactivar reconexión automática durante llamadas para evitar reconexiones
                        const shouldReconnect = false; // statusCode === 515 || statusCode === DisconnectReason?.restartRequired;
                        if (shouldReconnect && retries < maxRetries) {
                            console.log(`[BAILEYS-CALLER] Attempting reconnect (${retries + 1}/${maxRetries})`);
                            retries += 1;
                            setTimeout(connectSocket, 1000);
                        }
                        else {
                            console.log(`[BAILEYS-CALLER] Not reconnecting, shouldReconnect=${shouldReconnect}, retries=${retries}`);
    ...
    #handleAudioCaptureStart = () => {
        console.log(`[BAILEYS-CALLER] Audio capture START - audioSource: ${this.#activeCall?._audioSource}`);
        if (!this.#engine || !this.#capturePtr)
            return;
        const audioSource = this.#activeCall?._audioSource ?? "silence";
        console.log(`[BAILEYS-CALLER] Creating AudioFeeder with audioSource: ${audioSource}`);
        let audioChunkCount = 0;
        
        // Usar AudioFeederNoFFmpeg para streaming desde buffer en memoria
        if (audioSource === "stream") {
            console.log(`[BAILEYS-CALLER] Using AudioFeederNoFFmpeg for streaming`);
            this.#feeder = new AudioFeederNoFFmpeg(this.#captureSampleRate, this.#captureChannels, this.#captureFramesPerChunk, (chunk) => {
                if (this.#engine && this.#capturePtr) {
                    this.#engine.sendAudioData(chunk, this.#capturePtr);
                    audioChunkCount++;
                    if (audioChunkCount % 100 === 0) {
                        console.log(`[BAILEYS-CALLER] Audio chunks sent: ${audioChunkCount}`);
                    }
                    this.#activeCall?._emitAudioOut(chunk);
                }
            }, null); // No file source, will read from memory
        } else {
        ...
        this.#feeder.start();
        console.log(`[BAILEYS-CALLER] AudioFeeder started`);
    };
    #handleAudioCaptureStop = () => {
        console.log(`[BAILEYS-CALLER] onAudioCaptureStop called - NOT stopping AudioFeeder to keep audio flowing`);
        // NO detener el AudioFeeder - el WASM engine llama esto incorrectamente al conectar
        // this.#feeder?.stop();
        // this.#feeder = null;
        if (this.#engine && this.#capturePtr) {
            try {
