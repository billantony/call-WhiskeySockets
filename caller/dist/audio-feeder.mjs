...
export class AudioFeeder {
    ...
    start = () => {
        console.log(`[AudioFeeder] Starting with source: ${this.source}`);
        if (this.#proc)
            return;
        ...
        console.log(`[AudioFeeder] Spawning ffmpeg with args:`, inputArgs);
        ...
        console.log(`[AudioFeeder] ffmpeg PID: ${this.#proc.pid}`);
        this.#proc.stdout.on("data", (chunk) => {
            // console.log(`[AudioFeeder] Received ${chunk.length} bytes from ffmpeg`);
        ...
        this.#proc.stderr.on("data", (chunk) => {
            console.error(`[AudioFeeder] ffmpeg stderr: ${chunk.toString().trim()}`);
        });
        this.#proc.on("exit", (code) => {
            console.log(`[AudioFeeder] ffmpeg exited with code=${code}`);
            if (code !== 0 && code !== null) {
                console.error(`[AudioFeeder] ffmpeg exited with code=${code}`);
            }
            this.#proc = null;
        });
        this.#nextEmitAtMs = 0;
    ...
    #resolveInputArgs = () => {
        ...
        // Agregar -stream_loop -1 para loop infinito del audio
    ...
    #flushOne = (chunkSamples) => {
        ...
        // console.log(`[AudioFeeder] Emitting chunk ${this.chunksEmitted} (queue: ${this.#queue.length}, underflow: ${this.underflowChunks})`);
