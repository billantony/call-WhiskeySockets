...
const ICE_RESTART_IDLE_THRESHOLD_MS = parseInt(process.env.CALL_ICE_RESTART_IDLE_THRESHOLD_MS || '60000');
...
export class RelayRtcTransport {
    ...
    send = (packet, ip, port) => {
        ...
        // ICE restart desactivado para evitar reconexiones después de conectar
    ...
    #getOrCreateEarlyConnection = (ip, port) => {
        ...
        const connection = this.#getOrCreateConnection(info);
        console.log(`[RELAY-TRANSPORT] ensureConnection called - state: ${connection.state}, id: ${connection.id}`);
        // Si la conexión ya está abierta, no reconectar - esto evita reconexiones durante llamadas
        if (connection.state === "open") {
            console.log(`[RELAY-TRANSPORT] Connection already open, skipping reconnect`);
            return Promise.resolve();
        }
        if (connection.state === "connecting") {
            console.log(`[RELAY-TRANSPORT] Connection already connecting, waiting`);
            return connection.connectPromise ?? Promise.resolve();
        }
        console.log(`[RELAY-TRANSPORT] Starting new connection`);
        const promise = this.#connect(connection);
        connection.connectPromise = promise;
        try {
            await promise;
            console.log(`[RELAY-TRANSPORT] Connection established successfully`);
        }
        catch (err) {
            console.error(`[RELAY-TRANSPORT] Connection failed:`, err);
        }
        finally {
